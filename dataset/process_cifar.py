# ref: https://github.com/TaesikGong/NOTE/blob/main/process_cifar.py
import os
import sys
import numpy as np
import pickle
...
def process_corrupted_data(cor_dir):
    ...
    for i, corruption in enumerate(corruptions):
        ...
        for severity in severities:
            ...
            new_data_dir = (
                "./corrupted/severity-" + str(severity) + "/" + corruption + ".npy"
            )
...
if __name__ == "__main__":
    ...
    home_dir = os.getcwd()
    cor_dir = f"{home_dir}/{cor_dirname}"
    org_dir = f"{home_dir}/{org_dirname}"

    process_corrupted_data(
        cor_dir
    )  # process corrupted data and save to the dedicated directory
    process_original_data(
        org_dir, dataset
    )  # process original data and save to the dedicated directory
